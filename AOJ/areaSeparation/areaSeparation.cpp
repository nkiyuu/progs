...
int main(){
  ...
  while(1){
    ...
    for(int i = 0; i < lines.size(); i++){
      points_now.clear();
      points_now.push_back(lines[i].first); points_now.push_back(lines[i].second);
      for(int j = 0; j < i; j++){
        int flag = 0;
        if(isecSS(lines[i].first, lines[i].second, lines[j].first, lines[j].second)){
          P tmp = crosspointLL(lines[i].first, lines[i].second, lines[j].first, lines[j].second) ;
          for(int k = 0; k < points_now.size(); k++){
            if(points_now[k] == tmp) flag = 1;
          }
          if(!flag) points_now.push_back(tmp);
        }
      }
      answer += points_now.size() - 1 ;
    }
