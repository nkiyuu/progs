...
int main(){
  while(1){
    ...
    for(int i = 0; i < n; i++){
      cin >> xst >> xen >> y;
      points.push_back(xst); points.push_back(xen); points.push_back(y);
      buildings.push_back(points);
    }
    ...
    for(int i = 0; i < buildings.size(); i++){
      for(int j = buildings[i][0]; j <= buildings[i][1]; j++){
        for(int k = 0; k <= buildings[i][2]; k++){
          grid_alt[j][k] += to_string(i);
        }
      }
    }
    ...
    for(int k= 0; k < n; k++){
      for(int i = 0; i < 53; i++){
        for(int j = 0; j < 60; j++){
          if(grid_alt[i][j].find(to_string(k)) != -1 && !(grid_alt[i - 1][j].find(to_string(k)) != -1 && grid_alt[i - 1][j + 1].find(to_string(k)) != -1 && grid_alt[i][j + 1].find(to_string(k)) != -1 && grid_alt[i + 1][j + 1].find(to_string(k)) != -1 && grid_alt[i + 1][j].find(to_string(k)) != -1)){
            grid[i][j + 20] = 1 ;
          }
        }
      }
    ...
    vector<P> crosspoints, t;
    for(int i = 0; i < 53; i++){
      t = {};
      for(int j = 0; j < 60; j++){
        if(grid[i][j]){
            t = crosspointLC(P (0,0), P (0, 1), P (i - 26,j - 20),D (rad));
            if(t.size() == 2){
              crosspoints.push_back(t[0]);
              crosspoints.push_back(t[1]);
            }
        }
      }
    }
    ...
    for(int i = 0; i < crosspoints.size(); i++){
      if(answer > crosspoints[i].imag()) answer = crosspoints[i].imag();
    }
    cout << fixed << setprecision(4) << answer + rad << "\n";
  }
